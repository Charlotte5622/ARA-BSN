...
class BaseTrainer():
    ...
    def _run_step(self):
        ...
        if torch.isnan(total_loss) or torch.isinf(total_loss):
            print(f"Warning: Loss is {total_loss}")
        ...
        if max_grad_norm > 1e3:
            print(f"Warning: Max gradient norm: {max_grad_norm}")
    ...
    def test_DND(self, img_save_path):
        ...
        denoise_srgb(wrap_denoiser, './dataset/DND', self.file_manager.get_dir(img_save_path+'/mat'))
    ...
    def load_checkpoint(self, load_epoch=0, name=None):
        ...
        try:
            saved_checkpoint = torch.load(file_name, map_location='cpu', weights_only=False)
        except TypeError:  # torch without the weights_only argument
            saved_checkpoint = torch.load(file_name, map_location='cpu')
    ...
    def _find_last_epoch(self):
        prefix, suffix = '%s_'%self.session_name, '.pth'
        checkpoint_list = os.listdir(self.file_manager.get_dir(self.checkpoint_folder))
        epochs = []
        for ckpt in checkpoint_list:
            if ckpt.startswith(prefix) and ckpt.endswith(suffix):
                stem = ckpt[len(prefix):-len(suffix)]
                if stem.isdigit(): epochs.append(int(stem))
